...
class Trainer:
    ...
    def __init__(self, args):
        ...
        # Datasets
        if args.train_pickle_path is not None:
        ...
        if args.valid_pickle_path is not None:
        ...
        if not args.no_keywords:
            self.logger.info('Create Seq2SeqKeywords model')
            self.model = Seq2SeqKeywords(
                pretrain_dir=args.pretrain_dir if self.args.use_bert else None,
    ...
    def train_epoch(self):
        ...
        pbar = tqdm(self.train_batches,
                    desc=f'[{self.tag}] [{self.device}] Train {self.epoch}', 
                    dynamic_ncols=True)
    ...
    def fit(self):
        ...
        while True:
            ...
            if self.valid_batches is not None and \
                (self.epoch <= 3 or self.epoch % 20 == 0 or self.epoch >= 100):
                pred_responses, pred_keywords = self.valid_epoch()
                self.valid_metric(pred_responses, pred_keywords)
    ...
    def load_and_metric(self, start_epoch, glove_path=None):
        ...
        while True:
            ckpt_path = os.path.join(self.checkpoints_dir, f'model-{self.tag}-epoch-{self.epoch}.pt')
            ...
            if self.valid_batches is not None:
                pred_responses, pred_keywords = self.valid_epoch()
                self.valid_metric(pred_responses, pred_keywords)  # NOTE: 1
            # break  # NOTE: 2
            self.epoch += 1
    ...
    def valid_epoch(self):
        ...
        pbar = tqdm(self.valid_batches, 
                    desc=f'[{self.tag}] [{self.device}] Valid {self.epoch}', 
                    dynamic_ncols=True)
        for batch in pbar:
            if self.has_keywords:
                ...
                batch_hyps, _ = self.valid_batch(batch)
                hyps.extend(batch_hyps)
    ...
    def valid_batch(self, batch):
        with torch.no_grad():
            if self.has_keywords and not self.args.valid_gt_kw:
                y_pred, kw_pred = self.model(mode='valid', x=batch.batch_x, k=None, max_len=self.max_decode_len)
            elif self.has_keywords and self.args.valid_gt_kw:
        ...
        #     text = self.get_case_text(batch, i, y_pred, kw_pred)
        #     texts.append(text)
        # with open(f'{self.tag}-{self.epoch}-cases.txt', 'a') as f:
        #     f.write('\n\n' + '\n\n'.join(texts))
    ...
    def metric_ppl(self, epoch):
        ...
        self.model.eval()
        ppl_list = []
        pbar = tqdm(self.valid_batches, dynamic_ncols=True,
                    desc=f'[{self.tag}] Valid PPL Epoch {self.epoch}')
        for batch_idx, batch in enumerate(pbar):
            with torch.no_grad():
                batch.batch_y = batch.batch_y.to(self.device)
                if self.has_keywords and not self.args.valid_gt_kw:
                    logits = self.model(mode='valid_ppl', x=batch.batch_x, k=None,
                        y=batch.batch_y, max_len=self.max_decode_len)
                elif self.has_keywords and self.args.valid_gt_kw:
                    batch.batch_k = batch.batch_k.to(self.device)
                    logits = self.model(mode='valid_ppl', x=batch.batch_x, k=batch.batch_k,
                        y=batch.batch_y, max_len=self.max_decode_len)
                else:
                    logits = self.model(mode='train', x=batch.batch_x, y=batch.batch_y)
            loss = self.loss_fn(input=logits, target=batch.batch_y)
            ...
            pbar.set_postfix({'loss': f'{loss:.5f}', 'ppl': f'{ppl:.5f}'})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metric_ppl', default=0, type=int)

    parser.add_argument('--no_bert', action='store_true')
    parser.add_argument('--no_keywords', action='store_true')
    parser.add_argument('--valid_gt_kw', action='store_true')
    parser.add_argument('--anneal', default='cos', choices=['cos', 'gt', 'pred'])
    ...
    args.use_bert = not args.no_bert
    args.has_keywords = not args.no_keywords
    if args.metric_from > 0 or args.metric_ppl > 0:
        args.train_pickle_path = None
    ...
    elif args.metric_ppl > 0:
        print('Metric PPL')
        trainer.metric_ppl(epoch=args.metric_ppl)
    else:
        trainer.fit()
