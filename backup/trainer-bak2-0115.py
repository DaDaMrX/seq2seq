...
class Trainer:
    ...
    def fit(self):
        ...
        while True:
            ...
            if self.valid_batches is not None and \
                (self.epoch <= 3 or self.epoch % 20 == 0 or \
                    (self.epoch >= 170 and self.epoch % 2 == 0)):
                pred_responses, pred_keywords = self.valid_epoch()
                self.valid_metric(pred_responses, pred_keywords)
    ...
    def load_and_metric(self, start_epoch, glove_path=None):
        ...
        while True:
            ckpt_path = os.path.join(self.checkpoints_dir, f'model-{self.tag}-{self.epoch}.pt')
    ...
    def valid_batch(self, batch):
        ...
        #     texts.append(text)
        # with open('kw-293-cases.txt', 'a') as f:
        #     f.write('\n\n' + '\n\n'.join(texts))
